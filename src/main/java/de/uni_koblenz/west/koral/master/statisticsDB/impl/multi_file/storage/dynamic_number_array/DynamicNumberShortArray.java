 ...
 *
 */
public class DynamicNumberShortArray implements DynamicNumberArray {
	...
	public DynamicNumberShortArray() {
		this(DEFAULT_CAPACITY, DEFAULT_EXTENSION_LENGTH);
	}

	public DynamicNumberShortArray(int initialCapacity, int extensionLength) {
	 ...
	 */
	public DynamicNumberShortArray(short[] array, int extensionLength) {
		this.array = array;
		this.extensionLength = extensionLength;
	...
	private void upgrade(int minCapacity, Caller caller) {
		...
		short[] newArray = createExtendedArray(minCapacity);
		if (StatisticsDBTest.SUBBENCHMARKS) {
	 ...
	 */
	private void copy(int srcIndex, int destIndex, int length, short[] target, Caller caller) {
		int last = (destIndex + length) - 1;
	...
	public void insertGap(int index, int length, Caller caller) {
		...
		if ((lastNeededIndex + 1) > array.length) {
			...
			short[] target = createExtendedArray(lastNeededIndex + 1);
			if (StatisticsDBTest.SUBBENCHMARKS) {
				subbenchmark(SubbenchmarkType.ALLOC, caller, System.nanoTime() - start);
				start = System.nanoTime();
			}
	...
	private int findLastUsedIndex(short[] array) {
	...
	public short[] getData() {
		short[] data = new short[lastUsedIndex + 1];
