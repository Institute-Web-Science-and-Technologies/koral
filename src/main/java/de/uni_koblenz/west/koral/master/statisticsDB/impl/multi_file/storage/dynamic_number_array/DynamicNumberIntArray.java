 ...
 *
 */
public class DynamicNumberIntArray implements DynamicNumberArray {
	...
	public DynamicNumberIntArray() {
		this(DEFAULT_CAPACITY, DEFAULT_EXTENSION_LENGTH);
	}

	public DynamicNumberIntArray(int initialCapacity, int extensionLength) {
	 ...
	 */
	public DynamicNumberIntArray(int[] array, int extensionLength) {
		this.array = array;
		this.extensionLength = extensionLength;
	...
	private void upgrade(int minCapacity, Caller caller) {
		...
		int[] newArray = createExtendedArray(minCapacity);
		if (StatisticsDBTest.SUBBENCHMARKS) {
	 ...
	 */
	private void copy(int srcIndex, int destIndex, int length, int[] target, Caller caller) {
		int last = (destIndex + length) - 1;
	...
	public void insertGap(int index, int length, Caller caller) {
		...
		if ((lastNeededIndex + 1) > array.length) {
			...
			int[] target = createExtendedArray(lastNeededIndex + 1);
			if (StatisticsDBTest.SUBBENCHMARKS) {
				subbenchmark(SubbenchmarkType.ALLOC, caller, System.nanoTime() - start);
				start = System.nanoTime();
			}
	...
	private int findLastUsedIndex(int[] array) {
	...
	public int[] getData() {
		int[] data = new int[lastUsedIndex + 1];
