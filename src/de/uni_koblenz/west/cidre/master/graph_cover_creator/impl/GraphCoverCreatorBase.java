...
public abstract class GraphCoverCreatorBase implements GraphCoverCreator {
	...
	/**
	 * The input graph is <code>rdfFiles</code> the triples or quadruples have
	 * to be assigned to at least one graph chunk. After assigning it, it has to
	 * be written into the according file
	 ...
	 * @param outputs
	 *            the {@link OutputStream}s that are used to write the output
	 *            files
	 * @param writtenFiles
	 *            has to be set to true, if a triple is written to a specific
	 *            file (chunk)
	 */
	protected abstract void createCover(RDFFileIterator rdfFiles,
			int numberOfGraphChunks, OutputStream[] outputs,
			boolean[] writtenFiles);

	protected void writeStatementToChunk(int targetChunk,
			int numberOfGraphChunks, Node[] statement, OutputStream[] outputs,
			boolean[] writtenFiles) {
		...
		graph.add(new Quad(
				encodeContainmentInformation(targetChunk, numberOfGraphChunks),
